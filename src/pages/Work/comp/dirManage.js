...
export default function FileManage(props) {
  ...
  const onStart = ()=>{
    ...
    checkedKeys.forEach((item)=>{
      if(tmp.findIndex(v=>item.indexOf(v)>-1)===-1){
        if(item !== 'root'){
          tmp.push(item.replace('root',''))
        }else{
          tmp.push('/')
        }
      }
      ...
      <div className="leftbox">
        <div className="leftfiles">
          <Row className="lefttopmy">
            ...
            <Button type='primary' style={{width:'300px',float:'right'}} disabled={checkedKeys.length===0} onClick={onStart} icon={<CaretRightOutlined />} className="mr-3 ml-3" >{getLocaleDesc('calc_all')}</Button>
